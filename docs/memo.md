...
- 試合データの保存先はlocalStorageであり、サーバーDBではない。
- 複数端末間の同期や本番運用には、API・DB・WebSocketなどのサーバー連携が必要。
- 保存済みlocalStorageデータは、アプリの初期データ変更後も残るため、データ確認時は必要に応じてブラウザーのlocalStorageを初期化する。

## 次の候補

- サーバーAPIとデータベースへの移行。
- WebSocketによる本当のリアルタイム同期。
- 管理者権限のサーバー側認証。
- 操作履歴・監査ログ。
